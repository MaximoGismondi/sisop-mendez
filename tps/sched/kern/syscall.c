...
// Gets the scheduling priority of an environment specified by `env_id`.
// The priority is copied into `priority`.
// Returns 0 on success, -E_BAD_ENV on error.
static int
sys_get_env_priority(envid_t env_id, size_t *priority)
{
	struct Env *env;
	int r;
	if (r = envid2env(env_id, &env, 0)) {
		return r;
	}
...
// Sets the scheduling priority of an environment specified by `env_id` to
// `new_priority`. if `new_priority` is less than MIN_PRIORITY or greater than
// the current priority, the syscall fails.
// - Returns 0 on success
// - Returns 1 if the syscall failed because `new_priority` is less than
//   MIN_PRIORITY or greater than the current priority.
// - Returns -E_BAD_ENV if `env_id` is invalid.
static int
sys_set_env_priority(envid_t env_id, size_t new_priority)
{
	struct Env *env;
	int r;

	if (r = envid2env(env_id, &env, 1)) {
		return r;
	}
	if (new_priority < MIN_PRIORITY || new_priority > env->env_priority) {
		return 1;
	}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_get_env_priority:
		return sys_get_env_priority((envid_t) a1, (size_t *) a2);
	case SYS_set_env_priority:
		return sys_set_env_priority((envid_t) a1, (size_t) a2);
	default:
		return -E_INVAL;
	}
}
